...
export const BlogHome = ({ url }) => {
  ...
  const generateArticleSchemas = () => {
    ...
    const schemas = cards.map((card) => ({
      ...
      author: {
        "@type": "Person",
        name: card?.author_name || "Top5Shots",
      },
      publisher: {
        "@type": "Organization",
        name: "Top5Shots",
      ...
      datePublished: card?.createdAt || new Date().toISOString(),
      dateModified:
        card?.updatedAt || card?.createdAt || new Date().toISOString(),
      mainEntityOfPage: {
        "@type": "WebPage",
        "@id": `https://yourdomain.com/${card?.category?.slug}/${card?.slug}`,
      },
    }));
